...
async def setupPlanningSender(bot: BotType, ctx: Interaction, channel_id: str):
    ...
    leagues: list[dict] = (
        bot.api.getLeagues(language).get("data", {}).get("leagues", [])
    )

    leagues.sort(
        key=lambda x: (
            SORT_ORDER.get(language, []).index(x.get("region", ""))
            if x.get("region", "") in SORT_ORDER.get(language, [])
            else -1
        )
    )
    await ctx.response.send_message(
        view=createViewForSelect(
            [
                {
