...
if __name__ == "__main__":
    from bot.utils.planningConst import *
else:
...
def drawSeparator(x: int, y: int, img_draw: ImageDrawType, padding_top: int):
    img_draw.line(
        (
            x + margin,
            y + icon_b + padding_top,
            x + separator_len,
            y + icon_b + padding_top,
        ),
        fill=white,
...
def drawHour(
    ...
    time = datetime.datetime.fromisoformat(time).astimezone(tz).strftime("%H:%M")
    img_draw.text(
        (x + margin, y + icon_b / 2 + padding_top), time, font=hour_font, fill=white
    )
...
def _pasteImg(
    ...
    try:
        ...
        img_draw.text((int(x), int(y)), replacement_text, font=font, fill=white)
...
def drawFooterLeague(x: int, y: int, img_draw: ImageDrawType, bo_size: int):
    ...
    img_draw.text(
        (x + content_x / 2 - w / 2, y + icon_b / 2), text, font=hour_font, fill=white
    )
...
def drawGame(
    ...
    for team in teams:
        if os.path.exists(f"assets/teamsIcons/{team['code']}.png"):
            img = Image.open(f"assets/teamsIcons/{team['code']}.png")
            if img.mode != "RGBA":
...
def drawHalfDayMatches(
    ...
    if day_part == "am":
        ...
        py = content_y - (
            len(datas) * (icon_b + margin)
            + bo_text_height * how_many_bo_not_1
    ...
    for data_index, data in enumerate(data_list):
        ...
        if last_league != data[0]["league"]:
            if bo_size > 1:
                drawFooterLeague(x, y - icon_b / 2 - margin, img_draw, bo_size)
            last_league = data[0]["league"]
        ...
        if len(data) == 1:
            ...
            drawMultipleGames(x, y, timetable, img_draw, data)
    if bo_size > 1:
        y = top + (icon_b + margin) * (data_index) + py + icon_b / 2
        drawFooterLeague(x, y, img_draw, bo_size)
...
def formatSchedules(data: list[dict]):
    ...
    data = [
        ...
        if schedule.get("type", "") != "show"
        and datetime.datetime.fromisoformat(schedule.get("startTime", ""))
        .astimezone(_tz)
        .date()
        in weekSchedules
    ]
    data.sort(key=lambda x: datetime.datetime.fromisoformat(x.get("startTime", None)))
    for index, schedule in enumerate(data):
        ...
        if hour.hour < 12 or (
            index > 0
            and data[index - 1].get("league", {})
            in [
                g[0].get("league", {})
                for g in weekGames.get(date.isoformat(), {}).get("am", {}).values()
            ]
            and data[index - 1].get("league", {}) == schedule.get("league", {})
        ):
            day_part = "am"
        else:
            day_part = "pm"
        weekGames.setdefault(date.isoformat(), {}).setdefault(day_part, {}).setdefault(
            hour.isoformat(), []
        ).append(schedule)
    return weekGames
